...
$jform.find('.js-errorbox-all').css('display','block').show();$jform.trigger('tildaform:aftererror');return!0};checkVerifyTildaCaptcha=function(event){if(event.origin.indexOf('http://static.tildacdn.com/js/forms.tildacdn.com')!=-1){if(event.data=='closeiframe'){$('#tildaformcaptchabox').remove();$('#js-tildaspec-captcha').remove();return}
...
input.data($.mask.dataName,function(){return $.map(buffer,function(c,i){return tests[i]&&c!=getPlaceholder(i)?c:null}).join('')});input.one("unmask",function(){input.off(".mask").removeData($.mask.dataName)}).on("http://static.tildacdn.com/js/focus.mask",function(){if(input.prop("readonly")){return}
clearTimeout(caretTimeoutId);var pos;focusText=input.val();pos=checkVal();caretTimeoutId=setTimeout(function(){if(input.get(0)!==document.activeElement){return}
writeBuffer();if(pos==mask.replace("?","").length){input.caret(0,pos)}else{input.caret(pos)}},10)}).on("http://static.tildacdn.com/js/blur.mask",blurEvent).on("http://static.tildacdn.com/js/keydown.mask",keydownEvent).on("http://static.tildacdn.com/js/keypress.mask",keypressEvent).on("http://static.tildacdn.com/js/input.mask paste.mask",function(){if(input.prop("readonly")){return}
setTimeout(function(){var pos=checkVal(!0);input.caret(pos);tryFireCompleted()},0)});if(chrome&&android)
{input.off('http://static.tildacdn.com/js/input.mask').on('http://static.tildacdn.com/js/input.mask',androidInputEvent)}
checkVal()})}})}))